...
class ActorCriticRMA(nn.Module):
    ...
    def __init__(self,  num_prop,
        ...
        # Value function
        critic_layers = mlp_factory(activation,num_prop+self.scan_encoder_output_dim+priv_encoder_output_dim+self.scan_encoder_output_dim,1,critic_hidden_dims,last_act=False)
...
class ActorCriticRmaTrans(nn.Module):
    ...
    def save_torch_jit_policy(self,path):
        action_demo_input = torch.rand(1,5,12)
        obs_demo_input = torch.rand(1,5,self.config.n_obs)
        model_jit = torch.jit.trace(self.actor_student_backbone,(obs_demo_input,action_demo_input))
        model_jit.save(path)

class ActorCriticTrans(nn.Module):
    is_recurrent = False
    def __init__(self,  num_prop,
                        num_scan,
                        num_critic_obs,
                        ...
                        init_noise_std=1.0,
                        **kwargs):
        super(ActorCriticTrans, self).__init__()
